...
class CustomerForm(ttk.Frame):
    ...
    def build_form(self):
        ...
        # City
        ttk.Label(self, text="City").grid(row=4, column=0, padx=5, pady=5, sticky="w")
        self.city = ttk.Entry(self, width=30)
        self.city.grid(row=4, column=1, padx=5, pady=5)

        # State
        ttk.Label(self, text="State").grid(row=5, column=0, padx=5, pady=5, sticky="w")
        self.state = ttk.Entry(self, width=30)
        self.state.grid(row=5, column=1, padx=5, pady=5)

        # Zip Code
        ttk.Label(self, text="Zip Code").grid(row=6, column=0, padx=5, pady=5, sticky="w")
        self.zipcode = ttk.Entry(self, width=30)
        self.zipcode.grid(row=6, column=1, padx=5, pady=5)

        # Country
        ttk.Label(self, text="Country").grid(row=7, column=0, padx=5, pady=5, sticky="w")
        self.country = ttk.Entry(self, width=30)
        self.country.grid(row=7, column=1, padx=5, pady=5)

        # Phone Number
        ttk.Label(self, text="Phone Number").grid(row=8, column=0, padx=5, pady=5, sticky="w")
        self.phone = ttk.Entry(self, width=30)
        self.phone.grid(row=8, column=1, padx=5, pady=5)

        # Save button
        ttk.Button(self, text="Save Customer", command=self.save_customer).grid(row=9, column=0, columnspan=2, pady=10)
    ...
    def save_customer(self):
        ...
        cursor.execute(
            ...
            (first_name, last_name, email, address, city, state, zipcode, country, phone) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                self.first_name.get(),
        ...
        for field in [
            ...
            self.zipcode, self.country, self.phone
        ]:
